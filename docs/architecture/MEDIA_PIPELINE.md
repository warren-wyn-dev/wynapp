...
Attaching media is an API transaction, never a client promise. It locks the Drop row (or uses serializable/advisory locking), verifies ownership/current authorization, verifies every media record is READY, unattached, owned by the same subject and issued for the compatible purpose, rejects duplicate IDs, and calculates existing active attachments + requested attachments ≤ 9. It inserts ordered `drop_media` rows with a unique `(drop_id, position)`, unique active `media_id`, and position constraint `0..8`, then commits with the Drop change/outbox event. Concurrent attach/retry therefore cannot exceed nine. A database trigger or equivalent deferred constraint must guard the aggregate invariant because a simple row CHECK cannot count siblings.
