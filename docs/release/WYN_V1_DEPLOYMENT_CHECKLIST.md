...
| Sentry project (or any DSN-compatible ingest)                                                                                                                  | API, Worker, Web, Admin                               | Backend wired via `OBSERVABILITY_DSN` (see below); Web/Admin wired via `NEXT_PUBLIC_SENTRY_DSN`/`SENTRY_DSN` — same project, different variable names because Next.js requires the client-visible one to be `NEXT_PUBLIC_`-prefixed. |
...
| `NEXT_PUBLIC_SENTRY_DSN` | recommended | `https://<key>@o0.ingest.sentry.io/0`                                                                                                                                                                                                                                                                                                                                                                                            | Must be `NEXT_PUBLIC_`-prefixed — Next.js only inlines that-prefixed vars into the client bundle. Missing it is a soft fallback (the SDK stays inert), not a build or runtime failure. Covers unhandled client exceptions, React render errors (`app/error.tsx`, `app/global-error.tsx`), and server component/route errors. |
| `SENTRY_DSN`             | no          | Same value as `NEXT_PUBLIC_SENTRY_DSN`.                                                                                                                                                                                                                                                                                                                                                                                          | Server-only override for the server/edge runtime init; falls back to `NEXT_PUBLIC_SENTRY_DSN` if unset, so in practice only `NEXT_PUBLIC_SENTRY_DSN` needs to be set.                                                                                                                                                        |
...
| `NEXT_PUBLIC_SENTRY_DSN` | recommended | Same as `apps/web`'s.                                                                                                                         | Can be the same or a different Sentry project — separating Admin's error stream from Consumer's is a reasonable choice, not a requirement. |
| `SENTRY_DSN`             | no          | Same as `apps/web`'s — falls back to `NEXT_PUBLIC_SENTRY_DSN`.                                                                                |                                                                                                                                            |
