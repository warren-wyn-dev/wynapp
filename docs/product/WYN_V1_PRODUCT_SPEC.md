...
- ขอบเขตของ active session สำหรับ Logout รวม credential ทุกชนิดที่ server ออกให้แก่ login/session เดียวกัน (รวม access credential ที่ยังไม่หมดอายุและ refresh/renewal credential) ไม่ใช่เฉพาะ credential ที่ client ส่งมากับคำขอ; server ต้องคงสถานะการเพิกถอนให้อยู่หลัง response สำเร็จและปฏิเสธ request ที่กำลังแข่งหรือมาถึงภายหลังด้วย credential ในขอบเขตนั้น
